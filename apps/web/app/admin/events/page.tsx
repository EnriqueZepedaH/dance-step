...
export default async function AdminEventsPage() {
  ...
  const [{ data: events, error }, { data: sources }] = await Promise.all([
    supabase
      .from("events")
      .select("id, title, starts_at, kind, source, status, venues(name, neighborhood)")
  ...
  const sourceNames = new Map(
    (sources ?? []).map((s) => [s.key, s.display_name]),
  );

  return (
    <section className="page-shell">
      <span className="eyebrow bullet">Admin · Events</span>
      <h1 className="display">Events ({rows.length})</h1>
            ...
            <tbody>
              {rows.map((e) => (
                <tr key={e.id}>
                  ...
                  <td>
                    {e.source ? (
                      <span className="pill pill-mute" title={`source key: ${e.source}`}>
                        via {sourceNames.get(e.source) ?? e.source}
                      </span>
                    ) : (
                      <span className="pill pill-mute">admin</span>
                    )}
                  </td>
                  <td>
                    {e.status === "archived" ? (
                      <span className="pill pill-warn">archived</span>
                    ) : e.status === "hidden" ? (
                      <span className="pill pill-mute">hidden</span>
                    ) : (
